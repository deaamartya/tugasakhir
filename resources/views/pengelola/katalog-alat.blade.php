                        ...
                        <table id="example5" class="display" style="min-width: 845px">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td> {{ $d->NAMA_KATALOG_ALAT }} </td>
                                    <td>
                                        <div class="d-flex">
                                            <button type="button" class="btn btn-primary shadow btn-xs sharp mr-1" data-toggle="modal" data-target="#modal-edit-{{ $d->ID_KATALOG_ALAT }}"><i class="fa fa-pencil"></i></button>
                                            <button type="button" class="btn btn-danger shadow btn-xs sharp" data-toggle="modal" data-target="#modal-delete-{{ $d->ID_KATALOG_ALAT }}"><i class="fa fa-trash"></i></button>
                                        </div>												
                                    </td>											
                                </tr>
                                @endforeach
...
@foreach($katalogalat as $d)
{{-- Edit Modal --}}
<div id="modal-edit-{{ $d->ID_KATALOG_ALAT }}" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <form class="form-valide" action="{{ route('pengelola.katalog-alat.update',$d->ID_KATALOG_ALAT) }}" name="edit-katalogalat" method="POST" enctype="multipart/form-data" id="form-edit-{{ $d->ID_KATALOG_ALAT }}">
                    @method('PUT')
                    @csrf
                    <div class="form-group">
                        <label>Kategori Alat</label>
                        <select class="form-control select2" name="ID_KATEGORI_ALAT" id="ID_KATEGORI_ALAT">
                            @foreach($kategori as $t)
                                <option value="{{ $t->ID_KATEGORI }}" @if($d->ID_KATEGORI == $t->ID_KATEGORI) selected @endif>{{ $t->NAMA_KATEGORI }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
...
{{-- Delete Modal --}}
<div class="modal fade" id="modal-delete-{{ $d->ID_KATALOG_ALAT }}">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('pengelola.katalog-alat.destroy',[$d->ID_KATALOG_ALAT]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    Apakah anda yakin ingin menghapus ruang lab {{ $d->NAMA_KATALOG_ALAT }} ?
                    <div class="modal-footer">
...
$(document).ready(function(){
    $("#create-katalogalat").validate({
        ...
        messages: {
            ID_KATEGORI_ALAT: "Silahkan pilih tipe ruang lab",
            NAMA_ALAT: {
                required: "Silahkan isi Nama Katalog Alat pengguna",
            }
        },
        errorElement : 'div',
