    ...
    <div class="row page-titles mx-0">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0)">Data Laboratorium</a></li>
            </ol>
        </div>
    </div>
    <!-- row -->
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Buat Laboratorium Baru</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="form-validation">
                    <form id="create-lab" action="{{ route('admin.lab.store') }}" name="create-lab" method="POST">
                    @csrf
                        <div class="form-group">
                            <label>Nama Laboratorium</label>
                            <input type="text" class="form-control @error('NAMA_LABORATORIUM') is-invalid @enderror" id="NAMA_LABORATORIUM" name="NAMA_LABORATORIUM" required minlength="3" value="{{ @old('NAMA_LABORATORIUM') }}">
                            <div class="invalid-feedback animated fadeInUp">
                                Nama Laboratorium harus unik dengan minimal 3 karakter
                            </div>
                        </div>
...
@foreach($lab as $d)
{{-- Edit Modal --}}
<div id="modal-edit-{{ $d->ID_LABORATORIUM }}" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Laboratorium #{{ $d->ID_LABORATORIUM }}</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="form-valide" action="{{ route('admin.lab.update',$d->ID_LABORATORIUM) }}" name="edit-lab" method="POST" enctype="multipart/form-data" id="form-edit-{{ $d->ID_LABORATORIUM }}">
                @method('PUT')
                @csrf
                    <div class="form-group">
                        <label>Nama Laboratorium</label>
                        <input type="text" class="form-control @error('NAMA_LABORATORIUM') is-invalid @enderror" id="NAMA_LABORATORIUM" name="NAMA_LABORATORIUM" required minlength="3" value="{{ $d->NAMA_LABORATORIUM }}">
                        <div class="invalid-feedback animated fadeInUp">
                            Nama Laboratorium harus unik dengan minimal 3 karakter
                        </div>
                    </div>
...
{{-- Delete Modal --}}
<div class="modal fade" id="modal-delete-{{ $d->ID_LABORATORIUM }}">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Delete Laboratorium #{{ $d->ID_LABORATORIUM }}</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.lab.destroy',$d->ID_LABORATORIUM) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    Apakah anda yakin ingin menghapus Laboratorium {{ $d->NAMA_LABORATORIUM }} ?
                    <div class="modal-footer">
...
$(document).ready(function(){
    $("#create-lab").validate({
        ...
        messages: {
            NAMA_LABORATORIUM: {
                required: "Silahkan isi nama Laboratorium",
                minlength: "Nama Laboratorium minimal 3 karakter"
            },
        },
        errorElement : 'div',
