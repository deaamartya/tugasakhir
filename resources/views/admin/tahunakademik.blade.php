    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Buat Tahun Akademik Baru</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="form-validation">
                    <form id="create-tahun-akademik" action="{{ route('admin.tahun-akademik.store') }}" name="create-tahun-akademik" method="POST">
                    @csrf
                        <div class="form-group">
                            <label>Nama Tahun Akademik</label>
                            <input type="text" class="form-control @error('tahun_akademik') is-invalid @enderror" id="tahun_akademik" name="tahun_akademik" required minlength="3" value="{{ @old('tahun_akademik') }}">
                            <div class="invalid-feedback animated fadeInUp">
                                Nama Tahun Akademik harus unik dengan minimal 3 karakter
                            </div>
                        </div>
...
@foreach($tahunakademik as $d)
{{-- Edit Modal --}}
<div id="modal-edit-{{ $d->ID_TAHUN_AKADEMIK }}" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Tahun Akademik #{{ $d->ID_TAHUN_AKADEMIK }}</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="form-valide" action="{{ route('admin.tahun-akademik.update',$d->ID_TAHUN_AKADEMIK) }}" name="edit-tahun-akademik" method="POST" enctype="multipart/form-data" id="form-edit-{{ $d->ID_TAHUN_AKADEMIK }}">
                @method('PUT')
                @csrf
                    <div class="form-group">
                        <label>Nama Tahun Akademik</label>
                        <input type="text" class="form-control @error('tahun_akademik') is-invalid @enderror" id="tahun_akademik" name="tahun_akademik" required minlength="3" value="{{ $d->TAHUN_AKADEMIK }}">
                        <div class="invalid-feedback animated fadeInUp">
                            Nama Tahun Akademik harus unik dengan minimal 3 karakter
                        </div>
                    </div>
...
{{-- Delete Modal --}}
<div class="modal fade" id="modal-delete-{{ $d->ID_TAHUN_AKADEMIK }}">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Delete Tahun Akademik #{{ $d->ID_TAHUN_AKADEMIK }}</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.tahun-akademik.destroy',$d->ID_TAHUN_AKADEMIK) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    Apakah anda yakin ingin menghapus Tahun Akademik {{ $d->TAHUN_AKADEMIK }} ?
                    <div class="modal-footer">
...
$(document).ready(function(){
    $("#create-tahun-akademik").validate({
        rules: {
            tahun_akademik: {
                required: true,
                minlength: 3
